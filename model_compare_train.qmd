...
## Training Notes
This version of the model (09/10/2025) changed Consultant to a OneHot Encoded variable. This is because consultants may do a different case mix, so their mean Case Duration is potentially not meaningful for comparison. I also reduced the smoothing in the Target Encoding of procedure this is because if a procedure only appears once in the training data which happens often I would like the target encoded value to be closer to that value than the global mean.
...
for ax, (model_name, errors) in zip(axes, actual_errors.items()):
    ...
    ax.set_ylabel('Proportion of Predictions')
    ax.set_title(f'Distribution of Actual Errors (Clipped): {model_name}')
