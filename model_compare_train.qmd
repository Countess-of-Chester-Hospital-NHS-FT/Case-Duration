...
This uses the best / most consistent modelling process identified in the previous section and then trains the model on the full dataset to produce the final model.
...
if retrain == True:
    ...
    joblib.dump(final_best_model, f'model{date.today()}.joblib')
```